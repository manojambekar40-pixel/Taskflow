...
**List** — `GET /tasks?project_id=1`

Response `200`:
```json
[
  { "id": 1, "title": "Design database schema", "priority": "high", "due_date": "tomorrow", "project_id": 1 },
  { "id": 4, "title": "Write API docs", "priority": "medium", "due_date": "next friday", "project_id": 1 }
]
```
...
Response `200`:
```json
{ "id": 4, "title": "Write API docs", "priority": "high", "due_date": "next friday", "project_id": 1 }
```
...
**Sorted list (custom insertion sort)** — `GET /tasks?project_id=1&sort=priority`

Response `200` (ascending by priority weight low=1, medium=2, high=3):
```json
[
  { "id": 3, "title": "Refactor auth module", "priority": "low", "due_date": "next friday", "project_id": 1 },
  { "id": 4, "title": "Write API docs", "priority": "medium", "due_date": "next friday", "project_id": 1 },
  { "id": 1, "title": "Design database schema", "priority": "high", "due_date": "tomorrow", "project_id": 1 }
]
```
...
Request body:
```json
{ "description": "Finish the report tomorrow urgent", "project_id": 1 }
```
Response `201`:
```json
{ "id": 5, "title": "Finish the report", "priority": "high", "due_date": "tomorrow", "project_id": 1 }
```
...
`USE_REAL_LLM` feature flag. The default configuration keeps this flag
...
5. Set environment variables in the Render dashboard: `DATABASE_URL`
   ...
   `USE_REAL_LLM`, `FRONTEND_URL`, `PYTHON_VERSION`.
...
```
n=   10 | insertion_sort comparisons=      22 | binary_search comparisons=   3 | linear_search comparisons=     7
n=  500 | insertion_sort comparisons=   39999 | binary_search comparisons=   1 | linear_search comparisons=   352
n= 3000 | insertion_sort comparisons= 1493234 | binary_search comparisons=  11 | linear_search comparisons=  2686
```
